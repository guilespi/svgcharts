...
(function () {
    ...
    function Linechart(paper, x, y, width, height, valuesx, valuesy, opts, min_value, max_value,
    		show_lines_labels, lines_labels_position, lines_labels_color, value_symbols, symbol_position) {
 ...
 ** 
 \*/
        ...
        for (i = 0, ii = valuesy.length; i < ii; i++) {
            ...
            path = [];
            var gap = (width - ((valuesx[0].length - 1) * kx))/2,
            	str_value, str_symbol;
            for (var j = 0, jj = valuesy[i].length; j < jj; j++) {
                ...
                if(show_lines_labels){
    				if(Object.prototype.toString.call(value_symbols[i]) === '[object Array]'){
    					...
    					str_symbol = value_symbols[j];
    				}
    				str_value = addCommasToNumber(valuesy[i][j]);
        			if(symbol_position == 'prefix'){
        				str_value = str_symbol.length > 0 ? str_symbol + ' ' + str_value : str_value;
        			} else {
        				str_value = str_symbol.length > 0 ? str_value + ' ' + str_symbol : str_value;
        			}
                	paper.text(X, (lines_labels_position == 'inside' ? Y : Y -15), str_value)
	        	  	.attr('font-size', '18')
	        	  	.attr('text-anchor', 'middle')
	        	  	.attr('fill', lines_labels_color)
                }
                if (opts.smooth) {
                    if (j && j != jj - 1) {
 ...
 **
 ...
 \*/
    Raphael.fn.linechart = function(x, y, width, height, valuesx, valuesy, opts, min_value, max_value, 
    		show_lines_labels, lines_labels_position, lines_labels_color, value_symbols, symbol_position) {
        return new Linechart(this, x, y, width, height, valuesx, valuesy, opts, min_value, max_value, 
        		show_lines_labels, lines_labels_position, lines_labels_color, value_symbols, symbol_position);
    }
