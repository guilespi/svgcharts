	...
	*/
			...
			map(node.attrs['text'].split('\n'), function(text, iterable, line) {
				...
				tags.push(tag(
					...
					reduce(
						...
						{ style: 'text-anchor: ' + (node.attrs["text-anchor"] ? node.attrs["text-anchor"] : 'middle') + '; ' + styleToString(style) + ';' }
						),
					node.matrix,
	...
	R.fn.toSVG = function() {
		var paper   = this,
			...
			viewBox = paper.canvas.attributes.getNamedItem('viewBox').value,
			direction = (paper.canvas.attributes.getNamedItem('direction') ? 'direction="' + paper.canvas.attributes.getNamedItem('direction').value + '"' : ''),
			svg = '<svg style="position: relative;" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="'+viewBox+'" version="1.1" ' + direction + '>';
